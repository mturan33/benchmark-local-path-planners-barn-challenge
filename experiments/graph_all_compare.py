...
files = {
    ...
    'lfh': 'metrics/metrics_summary_lfh.csv',
    'sac': 'metrics/metrics_summary_sac.csv',
    'applr': 'metrics/metrics_summary_applr.csv'
}
...
metrics_to_plot = [
    ...
    "My_Recovery_Count", "Path_Smoothness",
...
for metric in metrics_to_plot:
    ...
    plt.title(f"{metric} - Comparative (Normalized + Smooth)")
    plt.xlabel("Data Point (Normalized Sequential)")
    ...
    fig = px.line(
        ...
        y="Normalized",
        color="Algorithm",
        title=f"{metric} - Interactive (Normalized + Filtered)",
        labels={"Normalized": f"{metric} (Normalized)", "index": "Data"}
    )
