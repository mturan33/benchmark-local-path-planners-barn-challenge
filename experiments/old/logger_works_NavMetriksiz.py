...
log_dir_terminal = os.path.expanduser("~/3v3/experiments/terminal_logs")
os.makedirs(log_dir_terminal, exist_ok=True)
log_file_path = os.path.join(log_dir_terminal, f"terminal_{algo_name}_w{world_idx}_t{trial}.txt")

# sys.stdout ve stderr hem terminale hem dosyaya yönlendirilir
sys.stdout = sys.stderr = TeeLogger(log_file_path)
print(f"🔁 Terminal çıktısı hem ekrana hem dosyaya kaydediliyor: {log_file_path}")
