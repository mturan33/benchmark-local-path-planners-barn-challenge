...
def wait_for_logger_finish(proc, timeout=200):
    try:
        outs, errs = proc.communicate(timeout=timeout)
        print(f"[metrics_logger.py stdout]\n{outs.decode()}")
        print(f"[metrics_logger.py stderr]\n{errs.decode()}")
    except subprocess.TimeoutExpired:
        print("[metrics_logger.py] Logger 200sn içinde bitmedi, kill ediliyor!")
        proc.kill()
...
def run_with_metrics(algo, config, world_idx, trial=1):
        ...
        try:
            ...
            while True:
                ...
                if "Test finished" in decoded or "Navigation timeout" in decoded:
                    ...
                    # Ekstra 10 sn bekleme, olası ek çıktıların yakalanması için
                    time.sleep(10)
                    break
    ...
    # Logger'ın doğal olarak kapanmasını bekle!
