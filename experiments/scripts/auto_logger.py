...
def start_roscore():
    ...
    roscore_path = '/opt/ros/melodic/bin/roscore'
    env = os.environ.copy()
    env["PATH"] = "/opt/ros/melodic/bin:" + env.get("PATH", "")
...
print("[run_all_with_metrics] roscore kontrolü pas geçiliyor, elle açıldı varsayılıyor.")
roscore_proc = None
...
def wait_for_logger_finish(proc, timeout=100):
    try:
        outs, errs = proc.communicate(timeout=timeout)
        print(f"[logger.py stdout]\n{outs.decode()}")
        print(f"[logger.py stderr]\n{errs.decode()}")
    except subprocess.TimeoutExpired:
        print("[logger.py] Logger 100sn içinde bitmedi, kill ediliyor!")
        proc.kill()

def kill_old_processes():
    for proc_name in ["gazebo"]:
        os.system(f"pkill -f {proc_name}")
...
def run_with_metrics(algo, config, world_idx, trial=1):
    ...
    metrics_cmd = (
        ...
        f"python3 logger.py _algorithm_name:={algo}"
    )
    ...
    with open(result_txt, "w") as outfile:
        try:
            ...
            while True:
                ...
                print(decoded, end='')  # Burada direkt terminale yazdırıyoruz
                outfile.write(decoded)
                outfile.flush()
                if "Test finished" in decoded or "Navigation timeout" in decoded:
                    print(f"[{algo.upper()}] Test tamamlandı, ek olarak 10 sn bekleniyor...")
                    time.sleep(10)
                    break
            proc.wait(timeout=100)
        except subprocess.TimeoutExpired:
            print(f"[!] {algo.upper()} TIMEOUT! Process kill ediliyor.")
            proc.kill()
        except Exception as e:
            print(f"[{algo.upper()}] Hata: {e}")

    wait_for_logger_finish(metrics_proc, timeout=60)

    print(f"[✓] Metrics (summary) dosyası güncellendi: {METRICS_PATH}/metrics_summary_{algo}.csv")

    if not wait_for_file(summary_csv, timeout=20):
