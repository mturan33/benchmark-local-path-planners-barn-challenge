...
class MetricsLogger:
    def __init__(self):
        ...
        # **Burada terminal_log dosyasını doğru ayarla:**
        self.terminal_log_dir = os.path.expanduser("~/3v3/experiments/terminal_logs")
        self.teb_log_file = os.path.join(self.terminal_log_dir, f"run_{self.algo}_w{world_idx}.txt")
        ...
        self.no_progress_threshold = rospy.get_param("~no_progress_threshold", 2.0)
    ...
    def collision_callback(self, msg):
        if self.goal_received and msg.data:
    ...
    def odom_callback(self, msg):
        ...
        if self.goal is not None:
            ...
            x1, y1 = self.goal
            line_dist = self.point_line_distance(x0, y0, x1, y1, pos.x, pos.y)
        ...
        if self.last_position is not None:
            ...
            if moved > self.progress_epsilon:
                progress = True
        else:
            progress = True
        ...
        if self.start_time and (now - self.start_time) > self.timeout_seconds:
            self.end_time = now
    ...
    def _extract_navigation_metric(self, timeout=30.0, interval=0.5):
        navigation_metric = 0.0
        start_time = time.time()
        while (time.time() - start_time) < timeout:
            try:
                with open(self.teb_log_file, "r") as f:
                    lines = f.readlines()
                    for line in reversed(lines):
                        if "Navigation metric:" in line:
                            parts = line.strip().split(":")
                            if len(parts) == 2:
                                navigation_metric = float(parts[1].strip())
                                return navigation_metric  # Bulunca hemen dön
            except Exception as e:
                print(f"Navigation metric okunurken hata: {e}")
            time.sleep(interval)
        return navigation_metric  # Bulamazsa 0 döner
    ...
    def _write_summary(self):
        ...
        navigation_metric = self._extract_navigation_metric()
        ...
        with open(self.summary_file, "a", newline='') as file:
            ...
            writer.writerow([
                ...
                round(path_efficiency, 3), round(navigation_metric, 4),
                int(self.collision),
